...
app.controller('BeaverController', function() {
	...
	beaver.init = function() {
		beaver.state_count = 2;
		beaver.tape          = new Tape();
	  beaver.update_head_ui();
    ...
    $('.its-a-one').removeClass('its-a-one');
	}
  ...
  beaver.update_head_ui = function() {
	  $('.current-head-position').removeClass('current-head-position');
    var pos = beaver.tape.current.node_id;
    if (Math.abs(pos) < 256) {
	    $($('.tape-cell')[pos+256]).addClass('current-head-position');
    } 
  }

  beaver.update_cell_ui = function() {	  
    var pos = beaver.tape.current.node_id;
    if (Math.abs(pos) < 256) {
      var e =  $($('.tape-cell')[pos+256]);
      if (beaver.tape.current.value == 1) {
        e.addClass('its-a-one');
      } else {
        e.removeClass('its-a-one');	
      }	
    }
  }
